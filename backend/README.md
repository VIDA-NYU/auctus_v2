...
├── generate_v2_synthetic_data.py   # Generates synthetic test dataset (v2 schema)
├── opensearch_config.py             # OpenSearch client, mapping, and helper functions
├── data/
│   └── synthetic_datasets_v2.json  # Test dataset file (generated by generate_v2_synthetic_data.py)
└── README.md                        # This file
```
...
| **initialize_opensearch.py** | Standalone indexing utility; deletes/recreates `datasets_v2` index; generates 384-d embeddings via SentenceTransformer; bulk-indexes all documents |
| **generate_v2_synthetic_data.py** | One-off script to produce the test dataset; outputs `datasets_v2.json` with full atlas-profiler metadata |
...
2. Create/recreate the `datasets_v2` index (if needed)
...
```bash
python3 generate_v2_synthetic_data.py
```

**Output:** `backend/data/synthetic_datasets_v2.json` (10 test datasets with full profiler metadata)
...
2. Deletes the `datasets_v2` index (if it exists)
3. Creates a new `datasets_v2` index with:
...
4. Loads all datasets from `synthetic_datasets_v2.json`
...
```bash
curl -s http://localhost:9200/datasets_v2/_mapping?pretty | jq .
```

### View Index Stats

```bash
curl -s http://localhost:9200/datasets_v2/_stats | jq '.indices.datasets_v2.primaries.docs'
```
