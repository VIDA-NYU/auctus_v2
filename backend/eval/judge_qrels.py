"""LLM judge -> provisional graded qrels, under the anti-leakage code invariant.

For every pooled (query, dataset) pair the judge assigns a graded relevance
0/1/2. The judge sees ONLY the neutral bundle (title + profile + data sample) —
the description arm under test is PHYSICALLY ABSENT from the prompt (report F1b).
If the judge saw the arm, NDCG would systematically favour the description arms
and the whole evaluation would be void. The same ``assert_no_arm_leak`` invariant
the query generator uses is asserted on every judged dataset here.

Grades (transplanted from NTCIR / ACORDAR):
  0 = not relevant; 1 = partially / topically related; 2 = fully answers the query.
...
grade how well each dataset answers the query.

Grades:
- 2 = fully relevant: the dataset directly answers the query.
- 1 = partially relevant: topically related but does not fully answer it.
- 0 = not relevant.
...
Return STRICT JSON mapping each dataset number to its grade, e.g. \
{{"1": 2, "2": 0, "3": 1}}. No prose outside the JSON.
...
def main(argv: list[str] | None = None) -> int:
    ...
    out.write_text(json.dumps({
        "_provenance": "PROVISIONAL LLM-judge qrels (un-calibrated on NYC); "
